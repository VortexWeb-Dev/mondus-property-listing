...
<script>
    function prepareFilters(e) {
        ...
        const fieldMappings = {
            'city': '%ufCrm15City',
            'community': '%ufCrm15Community',
            'subCommunity': '%ufCrm15SubCommunity',
            'building': '%ufCrm15Tower',
            'reference': 'ufCrm15ReferenceNumber',
            'permit': 'ufCrm15ReraPermitNumber',
            'listing_title': '%ufCrm15TitleEn',
            'property_type': 'ufCrm15PropertyType',
            'saleRent': 'ufCrm15OfferingType',
            'listing_agent': '%ufCrm15AgentName',
            'developer': '%ufCrm15Developers',
            'listing_owner': '%ufCrm15ListingOwner',
            'landlordEmail': '%ufCrm15LandlordEmail',
            'landlordPhone': '%ufCrm15LandlordContact',
            'bedrooms': 'ufCrm15Bedroom',
            'bathrooms': 'ufCrm15Bathroom',
            'price': 'ufCrm15Price',
            'portal': 'portal'
        }
        ...
        if (filterParams['portal']) {
            if (filterParams['portal'] == 'PF') {
                filterParams['ufCrm15PfEnable'] = 1
            } else if (filterParams['portal'] == 'BAYUT') {
                filterParams['ufCrm15BayutEnable'] = 1
            } else if (filterParams['portal'] == 'DUBIZZLE') {
                filterParams['ufCrm15DubizzleEnable'] = 1
            } else if (filterParams['portal'] == 'WEBSITE') {
                filterParams['ufCrm15WebsiteEnable'] = 1
            }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const fetchDevelopers = async () => {
            try {
                const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${DEVELOPERS_ENTITY_ID}&select[0]=id&select[1]=ufCrm11DeveloperName&order[ufCrm11DeveloperName]=asc`);
                const data = await response.json();
                const totalDevelopers = data.total;
                const developers = [];

                for (let i = 0; i < Math.ceil(totalDevelopers / 50); i++) {
                    const paginatedResponse = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${DEVELOPERS_ENTITY_ID}&select[0]=id&select[1]=ufCrm11DeveloperName&order[ufCrm11DeveloperName]=asc&start=${i * 50}`);
        ...
        const fetchAndDisplayOptions = async () => {
            try {
                ...
                createSelectOptions(developers, developerSelect, 'ufCrm11DeveloperName', 'ufCrm11DeveloperName');
