...
// Prepare data
$file_name     = !empty($property['ufCrm15ReferenceNumber'])
  ? sanitizeFileName($property['ufCrm15ReferenceNumber']) . ".pdf"
  : "Property_$id.pdf";

$title         = $property['ufCrm15TitleEn']       ?? "No Title";
$price         = !empty($property['ufCrm15Price']) ? "AED " . $property['ufCrm15Price'] : "Not Available";
$refId         = $property['ufCrm15ReferenceNumber'] ?? "";
$location      = $property['ufCrm15Location']      ?? "Unknown";
$description   = $property['ufCrm15DescriptionEn'] ?? "No Description";
$images        = $property['ufCrm15PhotoLinks']    ?? [];
$mainImage     = isset($images[0]) ? $images[0] : '';
$size          = $property['ufCrm15Size']          ?? "0";
$bedrooms      = $property['ufCrm15Bedroom']       ?? "0";
$bathrooms     = $property['ufCrm15Bathroom']      ?? "0";
$propertyType  = $property['ufCrm15PropertyType']  ?? "Unknown";
$offeringType = $property['ufCrm15OfferingType'] ?? "Unknown";
$availability  = $property['ufCrm15Availability']  ?? "Unknown";
$geopoints     = $property['ufCrm15Geopoints']     ?? null;

// Company info
$companyLogoPath = __DIR__ . "/assets/images/company-logo.png";
$companyName    = "Mondus Properties";
$companyAddress = "2402 Mondus Group Iris Bay - Business Bay - Dubai";
$companyWebsite = "https://mondusproperties.ae/";

// Agent/Owner info
if ($type === 'agent') {
  $agentName  = $property['ufCrm15AgentName']  ?? "Agent Name";
  $agentEmail = $property['ufCrm15AgentEmail'] ?? "info@mondusproperties.ae";
  $agentPhone = $property['ufCrm15AgentPhone'] ?? "+971 521110794";
} elseif ($type === 'owner') {
  $agentName  = $property['ufCrm15ListingOwner'] ?? "Owner Name";
  // Attempt to fetch owner details from Bitrix
  $userResponse = CRest::call("user.get", [
    "filter" => ["NAME" => $property['ufCrm15ListingOwner']]
  ]);
  $owner       = $userResponse['result'][0] ?? [];
  $agentEmail  = $owner["EMAIL"]          ?? "info@mondusproperties.ae";
  $agentPhone  = $owner["PERSONAL_MOBILE"] ?? "+971 521110794";
} else {
  ...
  $agentPhone = $user['PERSONAL_MOBILE'] ?? "+971 521110794";
}
